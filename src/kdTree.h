...
private:
    struct node{
        ...
        //por parametro
        double distance(const point& pt) const{return dist(pt, pto);}
    ...
    void cercano(const std::shared_ptr<node>& root, const point& nuevo, size_t index) {
        ...
        double dx = root->get(index) - getDimension(index, nuevo);
        index = (index + 1) % dimensiones;
...
public:
    ...
    std::list<point> cercanos(const point& pt, const std::list<point> listaPuntos, const int nPuntos){
        if (raiz == nullptr) {
            std::cerr << "tree is empty" << std::endl;
        }
        std::list<point> output;
        std::list<point> puntosAux = listaPuntos;
        ...
        for(int i = 0; i < nPuntos; i++){
            ...
            point puntoResul = arbolAuxiliar.mejorNodo->pto;
